...
# 2 game
    ...
    saved_actor_critic, ob_rms = torch.load('./trained_models/onegame/a2c/PongNoFrameskip-v4.pt')
    saved_actor_critic.to(device)
    saved_actor_critic.eval()
    ...
    for j in range(num_updates):
        ...
        value_loss, action_loss, dist_entropy, value_loss2, action_loss2, dist_entropy2 = agent.update(
            rollouts_load[j % len_rollouts_load], rollouts ,rollouts2)
        ...
        if j % args.log_interval == 0 and len(episode_rewards) > 1 and len(episode_rewards2) > 1:
